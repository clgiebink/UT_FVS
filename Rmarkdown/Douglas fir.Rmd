...
ggplot(data = glmm_data_df, aes(x = DIA_C, y = log(dds))) +
  geom_point(alpha=0.1) + geom_smooth(method = "lm")
...
old_fvs <- lm(log(dds)~SICOND+I(sin(ASPECT-0.7854)*SLOPE)+
                ...
                I(log(DIA_C))+I(BAL/100)+CR+I(CR^2)+PCCF,
              data = glmm_data_df)
...
full_model <- lmer(log(dds) ~ DIA_C + SICOND + SLOPE + ASPECT + 
                     BAL + PCCF + CCF + CR + wateryr + tmax_JanJun + 
                     (1 + DIA_C|TRE_CN) + (1|Year), data=glmm_data_df)
