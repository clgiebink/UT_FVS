...
```{r}
load("./data/formatted/con_df_z.Rdata")
```
...
```{r}
load("./data/formatted/conmt_df_z.Rdata")
```
...
```{r}
load("./data/formatted/var_df_z.Rdata")
```
...
```{r}
load("./data/formatted/varmt_df_z.Rdata")
```
...
#annual reduced
resid_annred <- residuals(an_red_df,type="pearson",scaled=TRUE)
...
#clim_16_red
resid_clmred <- residuals(clim_red_df,type="pearson",scaled=TRUE)
...
pred_annred <- predict(an_red_df, type="response")
...
pred_clmred <- predict(clim_red_df, type="response")
...
```{r}
plot_model(clim_red_df, type = "int", mdrt.values = "meansd")
plot_model(clim_rs, type = "int", mdrt.values = "meansd")
plot_model(clim_n_df, type = "int", mdrt.values = "meansd")
```
