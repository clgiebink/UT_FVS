...
A Variance Inflation Factor (VIF) above 3 shows strong collinearity. Crown ratio is above 3, and basal area of trees larger than the subject tree (BAL) and stand crown competition factor are close behind, suggesting they are correlated.
...
an_d_df <-  tidy(an_sdi_df) %>% 
  ...
  relabel_predictors(c(z.DIA_C = "DBH",                       # relabel predictors
                       ...
                       z.sin = "sin(Aspect-0.7854)*Slope",
                       z.cos = "cos(Aspect-0.7854)*Slope"))
comp_models_df <- full_join(an_rbal_df,an_kcr_df) %>%
  full_join(.,an_kccf_df) %>%
  full_join(.,an_d_df)
dwplot(comp_models_df, 
...
```{r}
AIC(an_redcr_df) #BAL & CCF
AIC(an_redbal_df) #CR & CCF
AIC(an_redblcf_df) #CR
AIC(an_ccf_df) #CCF
AIC(an_sdi_df) #SDI
```
...
Removing bal instead of CR from the model gives a better AIC score as well as brings all vif scores below 3. With VIF scores below 3, the model can be reduced based on significance or importance (i.e. coeficient value).

Before I reduce further, I will try another radiation parameter. We could also try substituting `sin` and `cos`, the radiation parameters, for a single solar radiation parameter, such as the Swift's solar radiation potential (1976).

```{r}
AIC(an_crsdi_df)
summary(an_crsdi_df)$coef
...
likelihood ratio test for slope on DBH
...
clim_red3_df <- lmer(log(dds)~
               ...
               data = glmm_df_z)
AIC(clim_red3_df)
anova(clim_red2_df,clim_red3_df)
```
...
mod_yr_df <- full_join(mod30_df,mod20_df) %>%
  full_join(.,mod10_df)
dwplot(mod_yr_df, 
       vline = geom_vline(xintercept = 0, colour = "grey60", linetype = 2), 
       # plot line at zero _behind_ coefs
...
r2beta(clim_mod_df)
r2beta(clim_mod5_df)
r2beta(clim_mod10_df)
```

```{r}
r2_30_df <- as.data.frame(r2beta(clim_mod_df))
r2_30_df$model <- "30 years"
r2_10_df <- as.data.frame(r2beta(clim_mod20_df))
r2_10_df$model <- "10 years"
r2_20_df <- as.data.frame(r2beta(clim_mod10_df))
r2_20_df$model <- "20 years"
r2_yr_df <- bind_rows(r2_30_df,r2_20_df)
r2_yr_df <- bind_rows(r2_yr_df,r2_10_df)
```

```{r}
#write.csv(r2_yr_df,file = "./data/formatted/r2_yr_df.csv")
#edit effect names and reload
#r2_yr_df <- read_csv("./data/formatted/r2_yr_df.csv")
```

```{r}
ggplot(data = r2_yr_df,aes(color = model)) + 
  geom_pointrange(data = r2_yr_df,aes(x = Effect, y = Rsq,ymin = lower.CL, ymax = upper.CL)) +
  scale_color_manual(values = c("30 years" = "tomato","20 years" = "limegreen","10 years" = "cornflowerblue"))
ggplot(data = r2_yr_df,aes(x = Effect, y = Rsq,color = model)) + 
  geom_point()
```
...
```{r}
full_mod_df <- full_join(full_an_df,full_clim_df)
dwplot(full_mod_df, 
       vline = geom_vline(xintercept = 0, colour = "grey60", linetype = 2), 
       # plot line at zero _behind_ coefs
       dot_args = list(aes(shape = model)),
  ...
  theme(plot.title = element_text(face="bold"),
        ...
        legend.background = element_rect(colour="grey80"),
        legend.title.align = 0.5)
red_mod_df <- full_join(red_an_df,red_clim_df)
dwplot(red_mod_df, 
       vline = geom_vline(xintercept = 0, colour = "grey60", linetype = 2), 
       # plot line at zero _behind_ coefs
       dot_args = list(aes(shape = model)),
