...
A Variance Inflation Factor (VIF) above 3 shows strong collinearity. There are no parameters above 3, but crown ratio (CR_weib) and basal area of trees larger than the subject tree (BAL) appear to be mildly correlated. In addition, crown competition factor of the subplot (PCCF) and the stand (CCF) as well as slope and slope squared seem to be mildly correlated.
...
There are no parameters above 3, but crown ratio (CR_weib) and basal area of trees larger than the subject tree (BAL) appear to be mildly correlated. In addition, crown competition factor of the subplot (PCCF) and the stand (CCF) as well as slope and slope*^2* seem to be mildly correlated.
...
Also, it may not be necessary to include three competition factors: CR, CCF and BAL. We can reduce one based on model coeficients and significance.

```{r}
#reduce
#CR2
#competiton?
clim_cr_es <- lmer(log(dds)~
                 #tree variables
                 z.DIA_C+I(z.DIA_C^2)+ #remove log due to standardization
                 z.CR_weib+
                 #climate
...
clim_mod20_es <- lmer(log(dds)~
               ...
               data = glmm_es_10)
AIC(clim_mod20_es)
summary(clim_mod20_es)$coef
```


```{r}
glmm_es_20 <- glmm_es_z %>%
  filter(Year >= 1972)
```
...
mod_yr_es <- full_join(mod30_es,mod20_es) %>%
  full_join(.,mod10_es)
dwplot(mod_yr_es, 
...
r2beta(clim_mod_es)
r2beta(clim_mod5_es)
r2beta(clim_mod10_es)
```

```{r}
r2_30_es <- as.data.frame(r2beta(clim_mod_es))
r2_30_es$model <- "30 years"
r2_10_es <- as.data.frame(r2beta(clim_mod20_es))
r2_10_es$model <- "10 years"
r2_20_es <- as.data.frame(r2beta(clim_mod10_es))
r2_20_es$model <- "20 years"
r2_yr_es <- bind_rows(r2_30_es,r2_20_es)
r2_yr_es <- bind_rows(r2_yr_es,r2_10_es)
```

```{r}
#write.csv(r2_yr_es,file = "./data/formatted/r2_yr_es.csv")
#edit effect names and reload
#r2_yr_es <- read_csv("./data/formatted/r2_yr_es.csv")
```


```{r}
ggplot(data = r2_yr_es,aes(color = model)) + 
  geom_pointrange(data = r2_yr_es,aes(x = Effect, y = Rsq,ymin = lower.CL, ymax = upper.CL)) +
  scale_color_manual(values = c("30 years" = "tomato","20 years" = "limegreen","10 years" = "cornflowerblue"))
ggplot(data = r2_yr_es,aes(x = Effect, y = Rsq,color = model)) + 
  geom_point()
```
...
#clim_16
resid_clm<- residuals(clim_16_es,type="pearson",scaled=TRUE)
...
pred_clm <- predict(clim_16_es, type="response")
...
#CR
plotLowess(resid_clmred~z.CR_weib,data = glmm_es_z,ylab="Residuals",main="Climate Reduced: CR")
#plotLowess(resid_lmm1~z.CR_weib,data = glmm_es_z,ylab="Residuals",main="LMM1")
#precip
...
plot(effect("z.DIA_C",clim_16_es),xlab = "DBH",main = "Full Climate")
...
```{r echo=FALSE, fig.width=3.5,fig.height=3}
par(mfrow=c(2,2))
plot(effect("z.CR_weib",annual_es),xlab = "Crown Ratio",main = "Full Annual")
plot(effect("z.CR_weib",an_red_es),xlab = "Crown Ratio",main = "Reduced Annual")
plot(effect("z.CR_weib",clim_16_es),xlab = "Crown Ratio",main = "Full Climate")
plot(effect("z.CR_weib",clim_red_es),xlab = "Crown Ratio",main = "Reduced Climate")
```
...
plot(effect("z.BAL",clim_16_es),xlab = "BAL",main = "Full Climate")
...
plot(effect("z.SICOND",clim_16_es),xlab = "Site Index",main = "Full Climate")
...
plot(effect("z.SLOPE",clim_16_es),xlab = "Slope",main = "Full Climate")
...
plot(effect("z.sin",clim_16_es),xlab = "sin(Aspect-0.7854)*Slope",main = "Full Climate")
...
plot(effect("z.cos",clim_16_es),xlab = "cos(Aspect-0.7854)*Slope",main = "Full Climate")
...
```{r echo=FALSE, fig.width=3.5,fig.height=3}
par(mfrow=c(1,2))
plot(effect("z.ppt_pJunSep",clim_16_es),xlab = "Precipitation",main = "Full Climate")
plot(effect("z.ppt_pJunSep",clim_red_es),xlab = "Precipitation",main = "Reduced Climate")
plot(effect("z.tmax_pAug",clim_16_es),xlab = "Temperature",main = "Full Climate")
plot(effect("z.tmax_pAug",clim_red_es),xlab = "Temperature",main = "Reduced Climate")
```
```{r echo=FALSE}
plot(effect("z.ppt_pJunSep:z.tmax_pAug",clim_16_es),xlab = "Interaction",main = "Full Climate")
