...
A Variance Inflation Factor (VIF) above 3 shows strong collinearity; therefore, crown ratio (CR_weib) and basal area of trees larger than the subject tree (BAL) appear to be strongly correlated. Slope and slope squared as well as crown competition factor of the subplot (PCCF) and the stand (CCF) seem to be mildly correlated.

```{r}
#reduce
#BAL
an_pp_red <- lmer(log(dds)~
                    #tree variables
                    z.DIA_C+I(z.DIA_C^2)+ #remove log due to standardization
                    z.CR_weib+I(z.CR_weib^2)+
                    #climate
...
Crown ratio (CR_weib) and basal area of trees larger than the subject tree (BAL) are correlated. Slope and slope squared as well as crown competition factor of the subplot (PCCF) and the stand (CCF) seem to be mildly correlated.
...
#CR2, SLOPE2
#competition
clim_cr_pp <- lmer(log(dds)~
                     #tree variables
                     z.DIA_C+I(z.DIA_C^2)+ #remove log due to standardization
                     z.CR_weib+
                     #climate
                     z.ppt_pJunSep+z.tmax_JunAug+ #not significant
                     #competition/density
                     #z.CCF+ #remove /100 due to standardization
                     #site variables
                     z.SICOND+z.SLOPE+
...
clim_crbal_pp <- lmer(log(dds)~
                   ...
                   data = glmm_pp_z)
AIC(clim_crbal_pp)
summary(clim_crbal_pp)$coef
clim_crcf_pp <- lmer(log(dds)~
                     #tree variables
                     z.DIA_C+I(z.DIA_C^2)+ #remove log due to standardization
                     z.CR_weib+
                       #climate
                   ...
                   data = glmm_pp_z)
AIC(clim_crcf_pp)
summary(clim_crcf_pp)$coef
clim_crsdi_pp <- lmer(log(dds)~
                     #tree variables
                     z.DIA_C+I(z.DIA_C^2)+ #remove log due to standardization
                     z.CR_weib+
                       #climate
...
clim_mod20_pp <- lmer(log(dds)~
                   ...
                   data = glmm_pp_10)
AIC(clim_mod5_pp)
summary(clim_mod5_pp)$coef
```

```{r}
glmm_pp_20 <- glmm_pp_z %>%
  filter(Year >= 1972)
```
...
mod30_pp <-  tidy(clim_mod_pp) %>% 
  filter(effect == "fixed") %>%
  select(term,estimate,std.error) %>%
  mutate(model = "30 years")%>%
...
mod25_pp <-  tidy(clim_mod5_pp) %>% 
  filter(effect == "fixed") %>%
  select(term,estimate,std.error) %>%
  mutate(model = "25 years")%>%
  relabel_predictors(c(z.DIA_C = "DBH",                       # relabel predictors
                       "I(z.DIA_C^2)" = "DBH^2",
                       z.CR = "Crown Ratio",
                       z.ppt_pJunSep = "Precipitation",
                       ...
                       z.cos = "cos(Aspect-0.7854)*Slope",
                       z.solrad_MayAug = "solrad"))
mod20_pp <-  tidy(clim_mod10_pp) %>% 
  filter(effect == "fixed") %>%
  select(term,estimate,std.error) %>%
  mutate(model = "20 years")%>%
  relabel_predictors(c(z.DIA_C = "DBH",                       # relabel predictors
                       "I(z.DIA_C^2)" = "DBH^2",
                       z.CR = "Crown Ratio",
                       "I(z.CR_weib^2)" = "CR^2",
...
mod_yr_pp <- full_join(mod30_pp,mod20_pp) %>%
  full_join(.,mod10_pp)
dwplot(mod_yr_pp, 
...
r2beta(clim_mod_pp)
r2beta(clim_mod5_pp)
r2beta(clim_mod10_pp)
```

```{r}
r2_30_pp <- as.data.frame(r2beta(clim_mod_pp))
r2_30_pp$model <- "30 years"
#r2_25_pp <- as.data.frame(r2beta(clim_mod5_pp))
#r2_25_pp$model <- "25 years"
r2_20_pp <- as.data.frame(r2beta(clim_mod10_pp))
r2_20_pp$model <- "20 years"
r2_10_pp <- as.data.frame(r2beta(clim_mod20_pp))
r2_10_pp$model <- "10 years"
r2_yr_pp <- bind_rows(r2_30_pp,r2_20_pp)
r2_yr_pp <- bind_rows(r2_yr_pp,r2_10_pp)
```

```{r}
#write.csv(r2_yr_pp,file = "./data/formatted/r2_yr_pp.csv")
#edit effect names and reload
#r2_yr_pp <- read_csv("./data/formatted/r2_yr_pp.csv")
```

```{r}
ggplot(data = r2_yr_pp,aes(color = model)) + 
  geom_pointrange(data = r2_yr_pp,aes(x = Effect, y = Rsq,ymin = lower.CL, ymax = upper.CL))+
  scale_color_manual(values = c("30 years" = "tomato","20 years" = "limegreen","10 years" = "cornflowerblue"))
ggplot(data = r2_yr_pp,aes(x = Effect, y = Rsq,color = model)) + 
  geom_point()
```
...
pred_annred <- predict(an_pp_red, type="response")
...
par(mfrow=c(2,2))
#dbh
plotLowess(resid_clmred~z.DIA_C,data = glmm_pp_z,ylab="Residuals",main="Climate Reduced: DBH")
#plotLowess(resid_lmm1~z.DIA_C,data = glmm_pp_z,ylab="Residuals",main="LMM1")
#CR
plotLowess(resid_clmred~z.CR_weib,data = glmm_pp_z,ylab="Residuals",main="Climate Reduced: CR")
#plotLowess(resid_lmm1~z.CR_weib,data = glmm_pp_z,ylab="Residuals",main="LMM1")
#precip
plotLowess(resid_clmred~z.ppt_pJunSep,data = glmm_pp_z,ylab="Residuals",main="Climate Reduced: Precip") 
#plotLowess(resid_lmm1~z.ppt_pJunSep,data = glmm_pp_z,ylab="Residuals",main="LMM1") 
#temp
plotLowess(resid_clmred~z.tmax_JunAug,data = glmm_pp_z,ylab="Residuals",main="Climate Reduced: Temp")
#plotLowess(resid_lmm1~z.tmax_JunAug,data = glmm_pp_z,ylab="Residuals",main="LMM1")
#CCF
plotLowess(resid_clmred~z.CCF,data = glmm_pp_z,ylab="Residuals",main="Climate Reduced: CCF")
#plotLowess(resid_lmm1~z.CCF,data = glmm_pp_z,ylab="Residuals",main="LMM1")
#SI
plotLowess(resid_clmred~z.SICOND,data = glmm_pp_z,ylab="Residuals",main="Climate Reduced: SI")
#plotLowess(resid_lmm1~z.SICOND,data = glmm_pp_z,ylab="Residuals",main="LMM1")
#Slope
plotLowess(resid_clmred~z.SLOPE,data = glmm_pp_z,ylab="Residuals",main="Climate Reduced: Slope")
#plotLowess(resid_lmm1~z.SLOPE,data = glmm_pp_z,ylab="Residuals",main="LMM1")
```
...
```{r}
full_mod_pp <- full_join(full_an_pp,full_clim_pp)
dwplot(full_mod_pp, 
       vline = geom_vline(xintercept = 0, colour = "grey60", linetype = 2), 
       # plot line at zero _behind_ coefs
       dot_args = list(aes(shape = model)),
  ...
  theme(plot.title = element_text(face="bold"),
        ...
        legend.background = element_rect(colour="grey80"),
        legend.title.align = 0.5)
red_mod_pp <- full_join(red_an_pp,red_clim_pp)
dwplot(red_mod_pp, 
       vline = geom_vline(xintercept = 0, colour = "grey60", linetype = 2), 
       # plot line at zero _behind_ coefs
       dot_args = list(aes(shape = model)),
...
plot(effect("z.DIA_C",an_pp_red),xlab = "DBH",main = "Reduced Annual")
...
```{r echo=FALSE, fig.width=3.5,fig.height=3}
par(mfrow=c(2,2))
plot(effect("z.CR_weib",annual_pp),xlab = "Crown Ratio",main = "Full Annual")
plot(effect("z.CR_weib",an_pp_red),xlab = "Crown Ratio",main = "Reduced Annual")
plot(effect("z.CR_weib",clim_16_pp),xlab = "Crown Ratio",main = "Full Climate")
plot(effect("z.CR_weib",clim_red_pp),xlab = "Crown Ratio",main = "Reduced Climate")
```
...
plot(effect("z.CCF",an_pp_red),xlab = "CCF",main = "Reduced Annual")
...
plot(effect("z.SICOND",an_pp_red),xlab = "Site Index",main = "Reduced Annual")
...
plot(effect("z.SLOPE",an_pp_red),xlab = "Slope",main = "Reduced Annual")
...
plot(effect("z.sin",an_pp_red),xlab = "sin(Aspect-0.7854)*Slope",main = "Reduced Annual")
...
plot(effect("z.cos",an_pp_red),xlab = "cos(Aspect-0.7854)*Slope",main = "Reduced Annual")
