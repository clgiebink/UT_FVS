...
glmm_data_pp <- data_all_pp %>%
  select(PLT_CN,TRE_CN, RW, dds, Year, DIA_C, 
        ...
        ppt_pAugOct, ppt_pOctDec, ppt_pNovJan, ppt_MayJul,
         ppt_pJunJan, wateryr, #JunJan should be AugJan
         tmax_Jun,
         tmax_JunAug) %>%
  filter(Year >= 1962)
...
A log transformation of dds skews the distribution to the left and doesn't get rid of outliers.

###Zeros and Missing Data

```{r}
length(unique(glmm_data_pp$TRE_CN[glmm_data_pp$Year == 1962]))
#[1] 51
```

My data is unbalanced. All trees have a growth measurement for 1962, but the last year of growth is not the same for all trees. Most trees end growth in 1994, but some end in 1992, 1993, or 1995. There are no missing data in between first and last year of growth for each tree, and there are no years of zero growth, indicating a missing ring.
...
```{r}
old_fvs <- lm(log(dds)~SICOND+I(sin(ASPECT-0.7854)*SLOPE)+I(cos(ASPECT-0.7854)*SLOPE)+
                SLOPE+I(SLOPE^2)+I(log(DIA_C))+I(BAL/100)+CR+I(CR^2)+I(DIA_C^2)+PCCF,
              data = glmm_data_pp)
...
```{r}
full_model <- lmer(log(dds) ~ DIA_C + SICOND + SLOPE + ASPECT + 
                     BAL + PCCF + CCF + CR + wateryr + tmax_JunAug + 
                     (1+DIA_C|TRE_CN) + (1|Year), data=glmm_data_pp)
...
```{r}
red_model <- lmer(log(dds) ~ DIA_C + SICOND + SLOPE + ASPECT + 
                     BAL + CCF + CR + wateryr + tmax_JunAug + 
                     (1+DIA_C|TRE_CN) + (1|Year), data=glmm_data_pp)
summary(red_model)$coef
```
