...
ggplot() + 
  ...
  geom_point(data=glmm_pp_z,aes(x=LON,y=LAT),colour="red")+
...
```{r}
par(mfrow=c(2,2))
hist(glmm_pp_z$RW,breaks = 50, 
     main = "Histogram of RW (mm)", xlab = "Increment")
hist(glmm_pp_z$dds,breaks = 100,
     main = "Histogram of DDS (in^2)", xlab = "Increment")
hist(log(glmm_pp_z$dds),breaks = 100, 
...
#Cleveland dotplot
par(mfrow=c(1,2))
dotchart(glmm_pp_z$dds)
dotchart(log(glmm_pp_z$dds))
```
...
```{r}
which(glmm_pp_z$RW == 0) #missing data
which(is.na(glmm_pp_z$RW))
which(glmm_pp_z$dds ==0)
...
length(unique(glmm_pp_z$PLT_CN))
```
...
growth_wateryr_plots_pp <- glmm_pp_z %>% 
  group_by(Year) %>%
  do(plots=ggplot(data=.) +
...
par(mfrow=c(2,2))
#detrend
ggplot(data = glmm_pp_z, aes(x = DIA_C, y = RW)) +
  geom_point(alpha=0.1) + geom_smooth(method = "lm")

ggplot(data = glmm_pp_z, aes(x = DIA_C, y = dds)) +
  geom_point(alpha=0.1) + geom_smooth(method = "lm")
#variance relatively constant

ggplot(data = glmm_pp_z, aes(x = DIA_C, y = log(dds))) +
  geom_point(alpha=0.1) + geom_smooth(method = "lm")

#log to reduce heteroscedasticity
ggplot(data = glmm_pp_z, aes(x = log(DIA_C), y = log(dds))) +
  geom_point(alpha=0.1) + geom_smooth(method = "lm")
#log makes it worse
...
```{r}
ggplot(data = glmm_pp_z, aes(x = SICOND, y = log(dds))) +
  geom_point(alpha=0.1) + geom_smooth(method = "lm")
```
...
```{r}
ggplot(data = glmm_pp_z, aes(x = SLOPE, y = log(dds))) +
  geom_point(alpha=0.1) + geom_smooth(method = "lm")
```
...
```{r warning=FALSE}
ggplot(data = glmm_pp_z, aes(x = BAL, y = log(dds))) +
  geom_point(alpha=0.1) + geom_smooth(method = "lm")
```
...
```{r}
ggplot(data = glmm_pp_z, aes(x = PCCF, y = log(dds))) +
  geom_point(alpha=0.1) + geom_smooth(method = "lm")
```
...
```{r}
ggplot(data = glmm_pp_z, aes(x = CCF, y = log(dds))) +
  geom_point(alpha=0.1) + geom_smooth(method = "lm")
```
...
```{r}
par(mfrow=c(1,2))
ggplot(data = glmm_pp_z, aes(x = CR, y = log(dds))) +
  geom_point(alpha=0.1) + geom_smooth(method = "lm")
ggplot(data = glmm_pp_z, aes(x = CR_fvs, y = log(dds))) +
  geom_point(alpha=0.1) + geom_smooth(method = "lm")
```
...
#CR
plotLowess(resid_clmred~z.CR_fvs,data = glmm_pp_z,ylab="Residuals",main="Climate Reduced: CR")
#plotLowess(resid_lmm1~z.CR_fvs,data = glmm_pp_z,ylab="Residuals",main="LMM1")
#precip
...
```{r}
sample_tre <- sample(glmm_pp_z$TRE_CN, size = 9)
