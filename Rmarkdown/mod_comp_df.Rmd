...
`CR_weib`*^2* can be removed based on significance. In addition, the exploration of the data didn't show any quadratic relationship between `log(dds)` and `CR_weib`. `PCCF` can be removed based on significance. In addition, there are several other competition or density factors in the model, such as `CCF` and `BAL`.
...
Again, `SLOPE` is collinear and should be removed.
...
models <- list(ran_slint = ran_slint, clim_1 = clim_1, clim_wy = clim_wy, clim_1_red = clim_1_red, clim_wy_red = clim_wy_red, glmm_g = glmm_g, glmm_red = glmm_red)
