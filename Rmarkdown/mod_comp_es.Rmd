...
load data
```{r}
load(file = 'C:/Users/clgie/OneDrive/Documents/Masters/Utah/UT_FVS/data/formatted/glmm_es_z')
```
...
clim_1 <- lmer(log(dds)~z.SICOND+I(sin(z.ASPECT-0.7854)*z.SLOPE)+
                    ...
                    I(log(z.DIA_C))+I(z.BAL/100)+z.CR_weib+
                     ...
                     (1+z.DIA_C|TRE_CN)+(1|Year),
                   data = glmm_es_z)
clim_2 <- lmer(log(dds)~z.SICOND+I(sin(z.ASPECT-0.7854)*z.SLOPE)+
                     I(cos(z.ASPECT-0.7854)*z.SLOPE)+z.SLOPE+
                    I(log(z.DIA_C))+I(z.BAL/100)+z.CR_weib+
                     I(z.DIA_C^2)+z.PCCF+I(z.CCF/100)+
                     z.wateryr+z.tmax_pAug+ #interaction not significant
                     (1+z.DIA_C|TRE_CN)+(1|Year),
                   data = glmm_es_z)
clim_3 <- lmer(log(dds)~z.SICOND+I(sin(z.ASPECT-0.7854)*z.SLOPE)+
                     I(cos(z.ASPECT-0.7854)*z.SLOPE)+z.SLOPE+
                    I(log(z.DIA_C))+I(z.BAL/100)+z.CR_weib+
                     I(z.DIA_C^2)+z.PCCF+I(z.CCF/100)+
                     z.wateryr+z.tmax_pJulSep+ #interaction not significant
                     (1+z.DIA_C|TRE_CN)+(1|Year),
                   data = glmm_es_z)

#reduce based on significance
clim_3_red <- lmer(log(dds)~z.SICOND+I(sin(z.ASPECT-0.7854)*z.SLOPE)+
                       I(cos(z.ASPECT-0.7854)*z.SLOPE)+z.SLOPE+
                       I(log(z.DIA_C))+I(z.BAL/100)+
                       I(z.DIA_C^2)+z.PCCF+
...
summarize models
```{r warning=FALSE}
models <- list(lm = lm, ran_slint = ran_slint, clim_1 = clim_1, clim_2 = clim_2, clim_3 = clim_3, clim_3_red = clim_3_red, glmm_g = glmm_g)
