  ...
  for(i in unique(newdata_rep$PLT_CN)) {
    ...
    while (growthyr <= (plt_df$fMEASYEAR[1]-1)){ #stops the year before fMEASYEAR
      ...
      for(i in 1:nrow(plt_yr_df)) {#for each tree on plot in given year
        ...
        z.DIA_C = unlist((plt_yr_df[i,"DIA"] - para_std[1,"DIA_C"]) / para_std[2,"DIA_C"])
        z.CR_fvs = unlist((plt_yr_df[i,"CR_fvs"] - para_std[1,"CR_fvs"]) / para_std[2,"CR_fvs"])
        z.BAL = unlist((plt_yr_df[i,"BAL"] - para_std[1,"BAL"]) / para_std[2,"BAL"])
        z.CCF = unlist((plt_yr_df[i,"CCF"] - para_std[1,"CCF"]) / para_std[2,"CCF"])
        z.PCCF = unlist((plt_yr_df[i,"PCCF"] - para_std[1,"PCCF"]) / para_std[2,"PCCF"])
        z.SDI = unlist((plt_yr_df[i,"SDI"] - para_std[1,"SDI"]) / para_std[2,"SDI"])
        z.SICOND = unlist((plt_yr_df[i,"SICOND_c"] - para_std[1,"SICOND"]) / para_std[2,"SICOND"])
        z.SLOPE = unlist((plt_yr_df[i,"SLOPE"] - para_std[1,"SLOPE"]) / para_std[2,"SLOPE"])
        z.sin = unlist((plt_yr_df[i,"sin"] - para_std[1,"sin"]) / para_std[2,"sin"])
        z.cos = unlist((plt_yr_df[i,"cos"] - para_std[1,"cos"]) / para_std[2,"cos"])
        z.solrad_MayAug = unlist((plt_yr_df[i,"solrad_MayAug"] - para_std[1,"solrad_MayAug"]) / para_std[2,"solrad_MayAug"])
        z.ppt_pJunSep = unlist((plt_yr_df[i,"ppt_pJunSep"] - 
        ...
        ##NA include none
        if(Species == 202) {
          z.tmax_FebJul = unlist((plt_yr_df[i,"tmax_FebJul"] -
                                    para_std[1,"tmax_FebJul"]) /
                                   para_std[2,"tmax_FebJul"])
          plt_yr_df$log_dds[i] <- summary(mod_df)$coef["(Intercept)",1] +
            summary(mod_df)$coef["z.DIA_C",1] * z.DIA_C +
            summary(mod_df)$coef["I(z.DIA_C^2)",1] * (z.DIA_C^2) +
            summary(mod_df)$coef["z.CR",1] * z.CR +
            summary(mod_df)$coef["z.ppt_pJunSep",1] * z.ppt_pJunSep +
            summary(mod_df)$coef["z.tmax_FebJul",1] * z.tmax_FebJul +
            summary(mod_df)$coef["z.SDI",1] * z.SDI +
            summary(mod_df)$coef["z.SICOND",1] * z.SICOND +
            summary(mod_df)$coef["z.SLOPE",1] * z.SLOPE +
            summary(mod_df)$coef["z.ppt_pJunSep:z.tmax_FebJul",1] * z.ppt_pJunSep * z.tmax_FebJul
        }
        if(Species == 122) {
          z.tmax_JunAug = unlist((plt_yr_df[i,"tmax_JunAug"] -
                                    para_std[1,"tmax_JunAug"]) /
                                   para_std[2,"tmax_JunAug"])
          plt_yr_df$log_dds[i] <- summary(mod_pp)$coef["(Intercept)",1] +
            summary(mod_pp)$coef["z.DIA_C",1] * z.DIA_C +
            summary(mod_pp)$coef["I(z.DIA_C^2)",1] * (z.DIA_C^2) +
            summary(mod_pp)$coef["z.ppt_pJunSep",1] * z.ppt_pJunSep +
            summary(mod_pp)$coef["z.tmax_JunAug",1] * z.tmax_JunAug +
            summary(mod_pp)$coef["z.BAL",1] * z.BAL +
            summary(mod_pp)$coef["z.SICOND",1] * z.SICOND +
            summary(mod_pp)$coef["z.SLOPE",1] * z.SLOPE +
            summary(mod_pp)$coef["z.ppt_pJunSep:z.tmax_JunAug",1] * z.ppt_pJunSep * z.tmax_JunAug
        }
        if(Species == 93) {
          z.tmax_pAug = unlist((plt_yr_df[i,"tmax_pAug"] -
                                               para_std[1,"tmax_pAug"]) / 
                                              para_std[2,"tmax_pAug"])
          plt_yr_df$log_dds[i] <- summary(mod_es)$coef["(Intercept)",1] +
            summary(mod_es)$coef["z.DIA_C",1] * z.DIA_C +
            summary(mod_es)$coef["I(z.DIA_C^2)",1] * (z.DIA_C^2) +
            summary(mod_es)$coef["z.CR",1] * z.CR +
            summary(mod_es)$coef["z.ppt_pJunSep",1] * z.ppt_pJunSep +
            summary(mod_es)$coef["z.tmax_pAug",1] * z.tmax_pAug +
            summary(mod_es)$coef["z.BAL",1] * z.BAL +
            summary(mod_es)$coef["z.SICOND",1] * z.SICOND +
            summary(mod_es)$coef["z.SLOPE",1] * z.SLOPE +
            summary(mod_es)$coef["z.ppt_pJunSep:z.tmax_pAug",1] * z.ppt_pJunSep * z.tmax_pAug
        }
