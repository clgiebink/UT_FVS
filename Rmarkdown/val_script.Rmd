...
save(pred_clim2,file = "/home/courtney/Documents/Masters/Research/Utah/UT_FVS/data/formatted/pred_clim2.Rdata")
```
...
```{r}
pred_clim_fvs <- pred_clim_fvs %>%
  group_by(TRE_CN) %>%
  mutate(DG_pred = max(DIA) - min(DIA),
         DG_obs = fDIA - min(DIA))
pred_cfvs_fmy <- pred_clim_fvs %>%
  filter(Year == fMEASYEAR) %>%
...
pred_n_fmy <- pred_clim_n %>%
  filter(Year == fMEASYEAR)
par(mfrow=c(1,2))
plot(pred_n_fmy$DIA,pred_n_fmy$fDIA,
     xlab = "Predicted", ylab = "Observed",
     main = "DBH")
plot(pred_n_fmy$DG_pred,pred_n_fmy$DG_obs,
     xlab = "Predicted", ylab = "Observed",
     main = "DG")
```
...
pred_rint_fmy <- pred_rint %>%
  filter(Year == fMEASYEAR)
par(mfrow=c(1,2))
plot(pred_rint_fmy$DIA,pred_rint_fmy$fDIA,
     xlab = "Predicted", ylab = "Observed",
     main = "DBH")
plot(pred_rint_fmy$DG_pred,pred_rint_fmy$DG_obs,
     xlab = "Predicted", ylab = "Observed",
     main = "DG")
```
...
fvs_check_red <- fvs_check_red %>%
  dplyr::select(-DIA) %>%
  mutate(DIA = eDBH)
mods_fmy <- list(cur_fvs = fvs_check_red,
                 #an_fvs = pred_afvs_fmy,
                 #an_full = pred_fan_fmy,
                 #an_red = pred_an_fmy,
                 #an2_red = pred_an2_fmy,
                 #clim_fvs = pred_cfvs_fmy,
                 #clim_full = pred_fclim_fmy,
                 #clim_red = pred_rclim_fmy, 
                 #clim2_red = pred_clim2_fmy,
                 #clim3_red = pred_clim3_fmy,
                 clim22_red = pred_clim22_fmy,
                 an_cr = pred_acr_fmy,
                 clim_cr = pred_ccr_fmy,
                 clim_cr2 = pred_ccr2_fmy,
                 clim_int = pred_int_fmy,
                 clim_n = pred_n_fmy,
                 clim_rs = pred_rs_fmy,
                 clim_rint = pred_rint_fmy,
                 clim_frs = pred_frs_fmy)
#mod_comp_dbh <- assess_dbh(mods_fmy = mods_fmy)
```
...
```{r}
mod_comp_dg <- assess_dg(mods_fmy = mods_fmy)
```
...
ggplot(data = mod_comp_dg, aes(x = RMSE, y = slope, label = mod_name)) +
  ...
  theme_bw()
ggplot(data = mod_comp_dg, aes(x = r2, y = slope, label = mod_name)) +
  geom_point(aes(size = RMSE)) + geom_text(hjust = 0, nudge_x = 0.0002) +
  theme_bw()
```
