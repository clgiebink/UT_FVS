...
r2beta(clim_red_df)
r2beta(clim_mod_df)
r2beta(clim_crfvs_df)
```

```{r}
#plot r2
r2_weib_df <- as.data.frame(r2beta(clim_red_df))
r2_weib_df$model <- "Weibull estimation"
r2_cr_df <- as.data.frame(r2beta(clim_mod_df))
r2_cr_df$model <- "Constant observation"
r2_crw_df <- as.data.frame(r2beta(clim_crfvs_df))
r2_crw_df$model <- "Weibull bounded"
r2_df <- bind_rows(r2_weib_df,r2_cr_df)
r2_df <- bind_rows(r2_df,r2_crw_df)
```
...
AIC(clim_red_pp)
r2beta(clim_mod_pp)
r2beta(clim_crfvs_pp)
AIC(clim_mod_pp)
```

```{r}
#plot r2
r2_weib_pp <- as.data.frame(r2beta(clim_red_pp))
r2_weib_pp$model <- "Weibull estimation"
r2_cr_pp <- as.data.frame(r2beta(clim_mod_pp))
r2_cr_pp$model <- "Constant observation"
r2_crw_pp <- as.data.frame(r2beta(clim_crfvs_pp))
r2_crw_pp$model <- "Weibull bounded"
r2_pp <- bind_rows(r2_weib_pp,r2_cr_pp)
r2_pp <- bind_rows(r2_pp,r2_crw_pp)
```
...
r2beta(clim_red_es)
r2beta(clim_mod_es)
r2beta(clim_crfvs_es)
```

```{r}
#plot r2
r2_weib_es <- as.data.frame(r2beta(clim_red_es))
r2_weib_es$model <- "Weibull estimation"
r2_cr_es <- as.data.frame(r2beta(clim_mod_es))
r2_cr_es$model <- "Constant observation"
r2_crw_es <- as.data.frame(r2beta(clim_crfvs_es))
r2_crw_es$model <- "Weibull bounded"
r2_es <- bind_rows(r2_weib_es,r2_cr_es)
r2_es <- bind_rows(r2_es,r2_crw_es)
```
...
df_stats <- glmm_df_z %>%
  ungroup() %>%
  dplyr::select(DIA_C,SICOND,tASPECT,SLOPE,BAL,SDI,CR_weib,CR,CR_fvs,PCCF,CCF,
...
```{r}
val_an_crb <- function(newdata,cr_int,mod_df,mod_pp,mod_es,sp_stats,nonfocal,bratio,ccf_df,CR_WEIB_df) {
  #parameters:
  #newdata - validataion trees from FIADB
  newdata_rep <- newdata %>%
    ungroup() %>%
...
```{r}
val_clim_crb <- function(newdata,mod_df,mod_pp,mod_es,sp_stats,nonfocal,bratio,ccf_df,CR_WEIB_df,climate) {
  #parameters:
  #newdata - validataion trees from FIADB
  newdata_rep <- newdata %>%
    ungroup() %>%
...
plot(val_dset$CR_weib,val_dset$PctCR,
...
```{r}
pred_clim_crb <- val_clim_crb(newdata = val_dset,mod_df = clim_crfvs_df, 
                              mod_pp = clim_crfvs_pp, mod_es = clim_crfvs_es,sp_stats = sp_stats,
                              nonfocal = non_focal, bratio = bratio_df ,ccf_df = ccf_df,
                              CR_WEIB_df = CR_WEIB_df, climate = val_clim)
```

```{r}
pred_clim_crb <- pred_clim_crb %>%
  group_by(TRE_CN) %>%
  mutate(DG_pred = max(DIA) - min(DIA),
         DG_obs = fDIA - min(DIA))
pred_climcri_fmy <- pred_clim_crb %>%
  filter(Year == fMEASYEAR)
par(mfrow=c(1,2))
plot(pred_climcri_fmy$DIA,pred_climcri_fmy$fDIA,
     xlab = "Predicted", ylab = "Observed",
     main = "DBH")
plot(pred_climcri_fmy$DG_pred,pred_climcri_fmy$DG_obs,
     xlab = "Predicted", ylab = "Observed",
     main = "DG")
fit_clim_cri <- lm(fDIA~DIA, data = pred_climcri_fmy)
summary(fit_clim_cri)
sqrt(mean(fit_clim_cri$residuals^2))
fit_clim_cri_dg <- lm(DG_obs~DG_pred, data = pred_climcri_fmy)
summary(fit_clim_cri_dg)
sqrt(mean(fit_clim_cri_dg$residuals^2))
```
