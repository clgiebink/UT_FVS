...
df_stats <- glmm_df_z %>%
  ungroup() %>%
  dplyr::select(DIA_C,SICOND,tASPECT,SLOPE,BAL,SDI,CR_weib,CR,PCCF,CCF,
...
```{r}
pred_clim_cr2 <- val_clim_cr(newdata = val_dset,mod_df = clim_mod_df, mod_pp = clim_mod_pp, 
...
pred_clim_cr2 <- pred_clim_cr2 %>%
  ...
  mutate(DG_pred = max(DIA) - min(DIA),
         DG_obs = fDIA - min(DIA))
pred_climcr2_fmy <- pred_clim_cr2 %>%
  filter(Year == fMEASYEAR)
par(mfrow=c(1,2))
