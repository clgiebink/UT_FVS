...
```{r}
pred_an_fvs <- pred_an_fvs %>%
  group_by(TRE_CN) %>%
  mutate(DG_pred = max(DIA) - min(DIA),
         DG_obs = fDIA - min(DIA))
pred_afvs_fmy <- pred_an_fvs %>%
  filter(Year == fMEASYEAR) %>%
  filter(!is.na(DIA))
```
...
```{r}
ggplot(data = mod_comp_dbh, aes(x = RMSE, y = slope, label = mod_name)) +
  geom_point(aes(size = r2)) + geom_text(hjust = 0, nudge_x = 0.002) +
  ylim(0.985, NA) +
  theme_bw()
```
